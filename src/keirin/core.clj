...
;; by Brent Boyer, the Criterium bench marking library for Clojure (see above)
;; and
...
(def ^:private default-reporting
  "other option is :underlying-results"
  :display-humane)

(def ^:private min-execution-time-threshold
  "%age of the min execution time that must be achieved"
  0.75)
...
(defn- median
  ...
  (let [sorted-data (sort data)
    ...
    (if (odd? len-data)
      ...
      (/ (+ (nth sorted-data (dec mid-point))
            (nth sorted-data mid-point)) 2.0))))
...
;;; We need an ultra fast place to store the result of running the "payload"
;;; function
...
(defn execute-many [f n]
  (if (= n 1)
    ...
    ;; Run it twice in succession - to allow JIT to do maximum
    ;; optimisations
    (loop [second-time false]
      (let [start (System/nanoTime)]
        (loop [i (long n)]
          (when (pos? i)
...
(defn- bench-one [payload gc-file-name n & {:keys [no-force-gc verbose]}]
  ...
  (let [compilation-bean
        (ManagementFactory/getCompilationMXBean)

        compilation-time-supported
        ...
        class-loaded-before
        (.getTotalLoadedClassCount class-loading-bean)

        class-unloaded-before
        (.getUnloadedClassCount class-loading-bean)

        compilation-time-before
        (and compilation-time-supported
             (.getTotalCompilationTime compilation-bean))

        gc-file-before
        (some-> gc-file-name read-gc-file)

        {:keys [result-hash-code time-taken]}
        (execute-many payload n)

        compilation-time-after
        (and compilation-time-supported
             (.getTotalCompilationTime compilation-bean))
        ...
        class-unloaded-after
        (.getUnloadedClassCount class-loading-bean)

        _
        (Thread/sleep 300) ;; Let GC file catch up

        gc-file-end
        (some-> gc-file-name read-gc-file)]

      {:gc-occurred
       (not= gc-file-before gc-file-end)

       :compilation-occurred
       (not= compilation-time-before compilation-time-after)

       :class-loading-occurred
       (or (not= class-loaded-before class-loaded-after)
...
(defn- iterate-bench
  [payload gc-file-name n & {:keys [verbose num-timed-runs] :as options}]
  (loop [gc-failure-count             0
    ...
    (let [{:keys [gc-occurred compilation-occurred class-loading-occurred
                  time-taken result-hash-code]}
          (apply bench-one payload gc-file-name n (-> options seq flatten))

          failure
          (or gc-occurred compilation-occurred class-loading-occurred)

          new-gc-failure-count
          (cond-> gc-failure-count gc-occurred inc)

          new-compilation-failure-count
          (cond-> compilation-failure-count compilation-occurred inc)

          new-class-loading-failure-count
          (cond-> class-loading-failure-count class-loading-occurred inc)

          new-iters
          (cond-> iters (not failure) inc)

          new-times
          (cond-> times
                  (not failure)
                  (conj {:time-taken       time-taken
      ...
      (when verbose
        (println "Run complete " time-taken "ms. GC occurred " gc-occurred
                 ...
                 ". Class loading occurred " class-loading-occurred
                 ". Result hash code " result-hash-code ". "))
...
(defn- warm-up!
  "Does warm-up-executions of benchmarked code and runs for at least
   warm-up-time seconds. Need to run for at least 10secs to ensure that the
   JVM gets the message and compiles code, rather than just interpret it."
  [payload verbose]
        ...
        (loop [i 0]
          ...
          (let [end-time (System/nanoTime)
            ...
            (if (and (>= i warm-up-executions)
                ...
                (recur (inc i)))))]
  (when verbose
    (println "JVM warmed up. Iterations " iterations
             ". Time taken " time "."))))

(defn- bench-one-set
  [payload gc-file-name n & {:keys [verbose num-timed-runs
                                    min-execution-time no-force-gc]
                             :or {num-timed-runs default-num-timed-runs}
                             :as options}]

  (let [{:keys [data gc-failures compilation-failures class-loading-failures]}
        (apply iterate-bench payload gc-file-name n
               (-> options
                   (merge {:num-timed-runs num-timed-runs})
                   seq
                   flatten))]
    ...
    (let [start-time (System/nanoTime)]
      ...
      (let [end-time (System/nanoTime)]
        ...
        (when (or (empty? data)
                  (not= (count data) num-timed-runs))
          (let [error-msg
                (str "Unable to complete " num-timed-runs " timed runs. "
        ...
        (let [times   (map :time-taken data)
              result  {:num-executions         n
                       ...
                       :MAD                    (MAD times)
                       :final-gc-time          (/ (- end-time start-time)
                                                  1000000000.0)
                       :data data}
              execution-time  (* (apply min times) n)]

          (when (and min-execution-time
                     (< execution-time (* min-execution-time
                                          min-execution-time-threshold)))
            (let [error-msg
                  (str "Did not run payload function for long enough to get"
                        " accurate results. Ran payload function for only "
                        execution-time "ms, when the minimum  required "
                       "execution time is " min-execution-time "ms.")]
              (throw (Exception. error-msg))))
          result)))))
...
(defn- display-humane
  [{:keys [timing-overhead median MAD]:as _results}]
  ...
  (when timing-overhead
    (let [as-percentage (* 100 (/ timing-overhead median))]
     (println "Timing loop overhead "
              (str (format "%3.2f" as-percentage)
                   "% ("
                   (humane-time timing-overhead)
                   ")")))))
...
(defn- estimate-num-executions
  "Estimate the number of times to execute the payload function in order to
   achieve the required minimum execution time.

   Timing an empty block with two successive calls to System/nanoTime on a
   2013 Macbook Pro can take as long as 300 nanos in the REPL and 150 nanos in
   compiled code. This introduces a considerable error for a function that only
   takes 10 nanos to run, but the error reduces as the number of executions
   increases.

   However, on the other hand slower functions may allocate a considerable
   amount of memory. If they are run too often then GCs may occur in the timing
   run invalidating.

   The strategy is to start with an initial estimate. If the minimum required
   execution time is not reached, then re-estimate based on the time executing
   this one function. The additional executions will reduce the timing error
   and likely lead to the payload function appearing to speed up. Therefore the
   new estimate will probably alsobe too low but more accurate. Eventually it
   will stabilize giving an accurate estimate.

   Critical that we get fairly accurate results - one slow run could throw the
   execution count right off - and impact results. So use min of 3 for each
   time"
  ...
  (loop [n 1]
    ...
    (let [{:keys [data gc-failures compilation-failures
                  class-loading-failures] :as result}
          (iterate-bench payload gc-file-name n
                         :num-timed-runs 3
                         :verbose verbose)

          _ (when-not (= 3 (count data))
              (let [error-msg
                    (str "Unable to get clean runs estimating number of "
                         "executions. GC failures: " gc-failures ". "
                         "Compilation failures: " compilation-failures "."
                         "Class loading failures: " class-loading-failures
                         ". ")]
                (throw (Exception. error-msg))))
          ...
          time-taken
          (->> data (map :time-taken) (apply min))

          time-taken
          (max time-taken 0e-6) ; In case we get back 0 for a fast function
      ...
      (if (> total-time-taken min-execution-time)
        ...
        (let [new-n (inc (int (quot min-execution-time time-taken)))
              ;; failsafe to ensure monotonically increasing
              new-n (max new-n (inc n))] ;
          (recur new-n))))))
...
(defn- calculate-timing-overhead
  ...
  (let [one   (int 1)
    ...
    (let [new-options
          (-> options
              (merge {:min-execution-time nil :no-force-gc true})
              seq
              flatten)

          results
          (apply bench-one-set dummy gc-file-name n new-options)
...
(defn bench*
  ...
  (let [gc-file-name  (get-gc-file-name)]
    (when-not gc-file-name
      (log/warn "JVM option: -Xlog:gc is not set. Keirin is unable to detect "
    ...
    (let [num-executions
          (estimate-num-executions payload gc-file-name
                                   min-execution-time verbose)
          ...
          new-options
          (-> options
              (merge {:min-execution-time min-execution-time})
              seq
              flatten)

          results
          (apply bench-one-set payload gc-file-name num-executions new-options)

          timing-overhead
          (when calc-timing-overhead
            (apply calculate-timing-overhead num-executions
                   gc-file-name new-options))

          results-with-overhead
          (cond-> results
                  timing-overhead
                  (assoc :timing-overhead timing-overhead))]
...
(defn quick-bench*
  [payload & {:keys [num-timed-runs]
              :or {num-timed-runs default-num-timed-runs-quick}
              :as options}]
  (apply bench* payload
         (-> options (merge {:num-timed-runs num-timed-runs}) seq flatten)))
